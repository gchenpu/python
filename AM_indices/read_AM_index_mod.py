...
def get_AM_index(p_levels=None):
    """
    get the annular mode indcies at `p_levels` (dtype='float32') using the class `Reanalysis`
    t: time index starting from 0
    y(time, pressure): annular mode indices
    p_levels: pressure levels
    """
...
#=============================================================
# class Reanalysis()
# Read the pre-processed geopotential height 
# and produce annular modes from reanalysis data
#=============================================================
class Reanalysis():
    def __init__(self, name_dir, year_start, year_end, name, output_freq='4xdaily', source_dir=None, \
        ...
        """
        Calculate the annular mode indices from pre-processed 4xdaily geopotential height averaged over the polar caps.
        The pre-processed data are
            NAM(time, pressure): Geopotential height averaged north of 65N.
            SAM(time, pressure): Geopotential height averaged south of 65S.
            GLOBAL(time, pressure): Globally averaged geopotential height.

        The class will remove the global mean height and normalized AM indices, and the indices are saved as 
        ...
        annual_cycle_fft: option to remove the harmonics set by `annual_cycle_fft` from the annual cycle. That is, the annual cycle for annual_cycle_fft=4 removes all the harmonics >= 4.  If annual_cycle_fft=0, all the harmonics are kept.
        ...
        if annual_cycle_fft != 4 or running_mean != 0:
            tag = f"_fft{annual_cycle_fft}_rm{running_mean}"
        else:
            tag = ""
        self.file_save = self.root_dir + '/' + self.name_dir + '/MODES_' + output_freq + '_' \
                    + str(self.year_start) + '-' + str(self.year_end) + tag + '.nc'
    ...
    def get_data(self, var_name):
        """
        read data from year_start to year_end by using the method `get_data_year`
        return var_o(year*days_in_a_year, pressure),    # total field
               var_mean_o(days_in_a_year, pressure),    # annual cycle, option to remove the harmonics >= `annual_cycle_fft`
               var_anomaly_o(year*days_in_a_year, pressure) # anomaly, option to conduct running average with `running_mean`
        """
    ...
    def cal_AM_index(self):
        """
        Calculating the annular mode indices from the original data using the method `get_data`
