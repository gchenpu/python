...
class LIM_core():
    def __init__(self, X, hyp_param, verbose=False):
        ...
        The LIM is constructed in `r` space. Mapping from `x` to `r` space and vice versa using get_r_from_x() and get_x_from_r() functions. Default is identity mapping, and can be modified by inheritance.
        """
        self.LIM_r(self.get_r_from_x(X), hyp_param, verbose=verbose)

        self.C0 = self.get_x_from_r(self.get_x_from_r(self.C0_r).T).T
        self.vl = self.get_x_from_r(self.vl_r.T).T
        self.vr = self.get_x_from_r(self.vr_r.T).T
        self.B = (self.vr @ np.diag(self.b) @ self.vl.conj().T).real
        self.Q = self.get_x_from_r(self.get_x_from_r(self.Q_r).T).T
        
    def LIM_r(self, X, hyp_param, verbose=False):
        """
        Input: X(years, days, r), where years and days are time (denoted by t), and r is space
        """
        lag = hyp_param['lim']['lag_time']

        self.C0_r = Myla.cov_lag(X, lag_time=0).astype(np.float64)
        Ct_r = Myla.cov_lag(X, lag_time=lag).astype(np.float64)
        Gt_r = Myla.pinv(self.C0_r, Ct_r, inv_method='pinv')

        self.w, self.vl_r, self.vr_r = Myla.eig(Gt_r, eig_method=hyp_param['lim']['eig_method'], verbose=False)
        # To ensure w is np.complex64; otherwise, for some cases, e.g., w = -0.0001, np.log(w) gives nan
        ...
        self.B_r = (self.vr_r @ np.diag(self.b) @ self.vl_r.conj().T).real

        # noise covariance matrix and eigenvectors
        # Q is a symmetric real square matrix and its eigenvalues are real
        self.Q_r = - self.B_r @ self.C0_r - self.C0_r @ self.B_r.T  # use matmul
        w_Q, vl_Q, vr_Q = Myla.eig(self.Q_r)
...
class LIM_forecast(LIM_core):
    ...
    def LIM_skills(self, t):
        ...
        for idx in range(len_t):
            Gt_r = self.get_Gt_r(t[idx])
            err[idx] = 1.0 - np.trace(Gt_r @ self.C0_r @ Gt_r.T)/np.trace(self.C0_r)
            R2[idx] = 1.0 - err[idx]
...
class LIM(LIM_diag, LIM_forecast):
    def __init__(self, X, hyp_param, verbose=False, use_LIM_skills=True):
        """
        Input: X(years, days, x), where years and days are time (denoted by t), and x is space
        Overriding get_r_from_x() and get_x_from_r() functions by mapping from `x` to `r` using SVD, where `r_opt` is the number of leading SVDs used in `r`
        """
        U_r, s, _ = Myla.svd(X)
...
def LIM_test(y, t, hyp_param):
    ...
    if __name__ == '__main__':
        print('saving tests/LIM_test.png ......')
        plt.savefig('tests/LIM_test.png')

#=============================================================
# function rand_model()
#=============================================================
def rand_model(B_r, vr_Q, w_Q, len_day=1000, dt=0.2, verbose=False, get_x_from_r=None, random_state=19680801):
    """
    LIM forced by random noise following Penland and Matrosova (1994)
        dX = X @ B.T * dt + dW @ Fs_dt_sqrt.T, where dW ~ N(0, dt)
        where X(t, x), and t is time and x is space
    or discretized as
        X[t+1,:] = X[t,:] + X[t, :] @ B.T * dt +  rng.standard_normal(len_Q) @ Fs_dt_sqrt.T * dt_sqrt
    The noise covariance matrix Q satisfies
        B @ C0 + C0 @ B.T + Q = 0, where C0 is the covariance of X at lag 0
        Q = Fs_dt_sqrt @ Fs_dt_sqrt.T
    Input:
        B_r[r, r]: the linear operator
        vr_Q, w_Q: eigenvectors and eigenvalues of the noise covariance matrix Q and Fs_dt_sqrt = vr_Q @ np.diag(w_Q**0.5)
        get_x_from_r: function to convert from r to x; if None, return X in r space
    return: X[t, r] or X[t, x] if get_x_from_r is provided
    """
    import math
    ...
    inv = 1.0 / dt
    step = int(round(inv))
    if not math.isclose(inv, step, rel_tol=0.0, abs_tol=1e-12):
        raise ValueError("`dt` must be the reciprocal of an integer (e.g. 0.25, 0.2, 0.1)")
    len_t = len_day * step
    dt_sqrt = dt**0.5

    len_r, len_Q = vr_Q.shape
    Fs_dt_sqrt = vr_Q @ np.diag(w_Q**0.5)

    X = np.empty((len_t+1, len_r))
    X[0, :] = rng.standard_normal(len_Q) @ Fs_dt_sqrt.T * dt_sqrt
    xi = np.empty((len_t, len_r))
    for t in range(len_t):
        xi[t,:] =  rng.standard_normal(len_Q) @ Fs_dt_sqrt.T
        X[t+1,:] = X[t,:] + X[t,:] @ B_r.T * dt + xi[t,:] * dt_sqrt

    if verbose:
        X_half = 0.5*(X[:-1,:] + X[1:,:])
        Q2 = (xi.T @ X_half + X_half.T @ xi)/len(X_half)/dt_sqrt
        print(f'Q2={Q2}')

    if get_x_from_r:
        return get_x_from_r(X[step::step])    # output once daily
    else:
        return X[step::step]    # output once daily

def rand_model2(B_r, Q_r, len_day=1000, dt=0.2, verbose=False, get_x_from_r=None, random_state=19680801):
    """
    A wrapper of rand_model() where Q_r is provided instead of its eigendecomposition
    Q_r is a real symmetric matrix with real eigenvalues and orthogonal eigenvectors
    """
    w_Q, vl_Q, vr_Q = Myla.eig(Q_r)
    w_Q, vl_Q, vr_Q = w_Q.real, vl_Q.real, vr_Q.real
    return rand_model(B_r, vr_Q, w_Q, len_day=len_day, dt=dt, verbose=verbose, get_x_from_r=get_x_from_r, random_state=random_state)
...
def rand_model_test():
    ...
    #=============================================================
    y3 = rand_model2(B, Q, len_day=100000, dt=0.2)
    # print(f'\ny3={y3.shape}')
    ...
    model1 = LIM_core(y3[None,:], hyp_param, verbose=True)
    model3 = LIM(y3[None,:], hyp_param, verbose=True)

    #=============================================================
    print('-'*10,'original parameters','-'*10)
    print(f'B={B},\n Q={Q},\n C0={C0},\n eigenvalues of B: {b}')
    # print(f'Check B @ C0 + C0 @ B.T + Q = 0:\n{B @ C0 + C0 @ B.T + Q}\n')

    #=============================================================
    print('-'*10,'diagnosed parameters from LIM core','-'*10)
    print(f'B={model1.B_r},\n Q={model1.Q_r},\n C0={model1.C0_r},\n eigenvalues of B: {model1.b}')

    #=============================================================
    print('-'*10,'diagnosed parameters from LIM','-'*10)
    print(f'B={model3.B},\n Q={model3.Q},\n C0={model3.C0},\n eigenvalues of B: {model3.b}')
...
class Myla():
    ...
    def eig(a, eig_method='eig2', inv_method='pinv', verbose=False):
        ...
        def check_eigs(w, vl, vr, a=None):
            ...
            if a is not None:
                ...
                if __name__ == "__main__":
                    print('saving tests/eig_m.png ......')
                    plt.savefig('tests/eig_m.png')
    ...
    def cov_lag(X, lag_time, X2=None):
        ...
        Ct = X1[:, :, lag_time:] @ X2[:, :len_day-lag_time, :] / (len_day-lag_time)
...
def Gt_test(X, hyp_param, verbose=False):
    ...
    """
    lag = hyp_param['lim']['lag_time']
    C0 = Myla.cov_lag(X, lag_time=0).astype(np.float64)
    Ct = Myla.cov_lag(X, lag_time=lag).astype(np.float64)
    ...
    if verbose:
        ...
        plt.colorbar()
        if __name__ == '__main__':
            print('saving tests/Gt_test.png ......')
            plt.savefig('tests/Gt_test.png')
...
def Myla_test(X, hyp_param):
    print('testing My linalg class ......')

    Gt_test(X, hyp_param, verbose=True)
    eig_m_test(X, hyp_param)
