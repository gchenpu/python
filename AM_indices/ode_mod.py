...
class OdeDataset(Dataset):
    def __init__(self, X, batch_time):
        ...
        self.X = X
        # X_idx[data_size, 2]: index for X
        self.X_idx = torch.from_numpy(np.array(np.nonzero(np.ones((len_year, len_day-batch_time)))).T)
    ...
    def __getitem__(self, idx):
        X = self.X[self.X_idx[idx,0], self.X_idx[idx,1]]
        y = self.X[self.X_idx[idx,0], self.X_idx[idx,1]:self.X_idx[idx,1]+self.batch_time]
        return X, y
...
def get_dataset(dataset, data_param, verbose=False):
        ...
        if verbose:
            ...
            print(f'X0.shape = {X0.shape} for (batch_size, x_size) \ny0.shape = {y0.shape} for (batch_size, batch_time, x_size)')

        return train_loader, test_loader

def test_dataset(X, hyp_param):
    print('Loading data ......')
    dataset = OdeDataset(X, hyp_param['data']['batch_time'])
    train_loader, test_loader = get_dataset(dataset, hyp_param['data'], verbose=True)
    ...
    def forward(self, X, t=None):    
        ...
        if t is not None:    
            ...
            if self.t_batch is None:
                ...
                # 'permute' switches the 0th dim (batch_time: integration in time) of 'y' with its 1st dim (batch_size: number of samples) for output
                return y.permute((1, 0, 2)) @ self.func.U.T

def test_OdeModel(hyp_param, lim, t, verbose):
    print(f"\nInitializing model ......")
    func = get_func(hyp_param['func'], lim, verbose=verbose)    # func: neural network
    # t_batch used for batch training without specifying 't'
    model = OdeModel(func, hyp_param['ode'], t_batch=t[:hyp_param['data']['batch_time']], verbose=verbose)
...
#=============================================================
class NeuralODE():
    def __init__(self, X, t, hyp_param, lim=None, verbose=False):
        """
        train the neural network `func` using the input dataset `X`
...
#=============================================================
# nn.Module class used to define func or net for f(t, y)
#=============================================================
def get_func(func_param, lim=None, verbose=False):
        # func
        if func_param['method'] == 'DM':
            if lim is not None:
                n_hidden = func_param["size"]
                func = DMFunc(lim, n_hidden, verbose=verbose)
            else:
                raise Exception('lim is not recognized!')
...
class DMFunc(nn.Module):
    def __init__(self, lim, n_hidden, verbose=False):
        super(DMFunc, self).__init__()
        self.U = lim.U
        self.vr_s = nn.Parameter(lim.vr_s)
        self.b = nn.Parameter(lim.b)

        self.net = TanhFunc(lim.vr_s.shape[1], n_hidden, lim.vr_s.shape[1]).net

    def update_B(self):
        self.vl_s = la.pinv(self.vr_s).conj().T
        self.vr = self.U.to(self.vr_s.dtype) @ self.vr_s
        self.vl = self.U.to(self.vr_s.dtype) @ self.vl_s

        b_clamp = self.b.clone()
        b_clamp.real = self.b.real.clamp(max=0)   # enforce negative real eigenvalue

        self.B_s = (self.vr_s @ torch.diag(b_clamp) @ self.vl_s.conj().T).real
        self.B = self.U @ self.B_s @ self.U.T

    def forward(self, t, y):
        return y @ self.B_s.T + self.net(y)
