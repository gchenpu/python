...
#=============================================================
class MyGetData():
    def __init__(self, data, annual_cycle_fft=2, running_mean=0):
        """
    ...
    def get_anomaly(self, var_name):
        ...
        ylen = var.shape[1]
        # var_o = var.reshape(-1, self.data.num_levels)    # dim(year*days_in_a_year, :)

        var_mean_o = MyGetData.cal_annual_cycle(var, self.annual_cycle_fft)

        var_anomaly = (var - var_mean_o).reshape(-1, self.data.num_levels)    # broadcasting the 1st dimension
        var_anomaly_o = MyGetData.cal_running_mean(var_anomaly, self.running_mean)

        return var_mean_o, var_anomaly_o.reshape(-1, ylen, self.data.num_levels)
    ...
    def get_slice(self, var_i, month_start, len_slice, slice_offset=0):
        ...
        my_slice /= my_slice[:,slice_offset:len_slice+slice_offset,:].reshape(-1, var.shape[1]).std(axis=0)    # `slice_offset` is not included for calculating std
    ...
    # @staticmethod
    # def cal_annual_cycle_test(var, annual_cycle_fft=4, annual_cycle_shift=False):
    #     """ 
    #     Input: var(year, days_in_a_year, :), where year and days_in_a_year are time, and `:` is space \\
    #     calculate the annual cycle of `var` by averaging in year \\
    #     annual_cycle_fft: option to remove the harmonics above `annual_cycle_fft` from the annual cycle
    #     annual_cycle_shift: shift the annual cycle by half prior to fft if True
    #     """

    #     var_mean = var.mean(axis=0)
    #     var_mean_o = np.empty_like(var_mean)
    #     var_mean_shift = np.empty_like(var_mean)
    #     var_mean_shift_o = np.empty_like(var_mean)

    #     if annual_cycle_fft > 0:
    #         if(annual_cycle_shift):
    #             num_days = var_mean.shape[0]
    #             num_days_half = round(num_days/2)
    #             # print(num_days, num_days_half)
    #             var_mean_shift[:num_days_half,:] = var_mean[-num_days_half:,:]
    #             var_mean_shift[num_days_half:,:] = var_mean[:num_days-num_days_half,:]
    #         else:
    #             var_mean_shift = var_mean

    #         var_mean_fft = np.fft.fft(var_mean_shift, axis=0)
    #         var_mean_fft[annual_cycle_fft+1:-annual_cycle_fft, :] = 0
    #         var_mean_shift_o = (np.fft.ifft(var_mean_fft, axis=0)).real

    #         if(annual_cycle_shift):
    #             var_mean_o[-num_days_half:,:] = var_mean_shift_o[:num_days_half,:]
    #             var_mean_o[:num_days-num_days_half,:] = var_mean_shift_o[num_days_half:,:]
    #         else:
    #             var_mean_o = var_mean_shift_o
    #     else:
    #         var_mean_o = var_mean
    ...
    def cal_running_mean(var, running_mean=0):
        """ 
        Input: var(year*days_in_a_year, :), where year*days_in_a_year is time, and : is space \\
...
class AM(MyGetData):
    def __init__(self, data, index_name, annual_cycle_fft=2, running_mean=0, save_index=False):
        ...
        if self.annual_cycle_fft == 2 and self.running_mean == 0:      # default values
            tag = ""
        else:
    ...
    def cal_index(self):
        ...
        AM_index = -(AM_anomaly-GLOBAL_anomaly).reshape(-1, self.data.num_levels)
        AM_index /= AM_index.std(axis=0)

        return AM_index
    
    def cal_slice(self, month_start, len_slice, slice_offset=0):

        var = self.index.reshape(-1, self.data.length_of_year, self.data.num_levels)
        
        return self.get_slice(var, month_start, len_slice, slice_offset)

#=============================================================
# NAM index class
#=============================================================
class NAM(AM):
    def __init__(self, data, index_name='NAM', annual_cycle_fft=2, running_mean=0, save_index=False):
...
#=============================================================
class SAM(AM):
    def __init__(self, data, index_name='SAM', annual_cycle_fft=2, running_mean=0, save_index=False):
...
def MyGetData_test():
    ...
    jra55 = JRA55(name_dir='jra_55', year_start=1958, year_end=2016, name='JRA55')
    jra55_index = NAM(jra55, annual_cycle_fft=2, running_mean=0)
    print(jra55_index.index.shape)

    CESM2 = CMIP6(name_dir='CESM2', year_start=1950, year_end=2014, source_dir='cmip6')
    CESM2_index = NAM(CESM2, annual_cycle_fft=2, running_mean=0)
    print(CESM2_index.index.shape)

    era5 = ERA5(name_dir='ERA5', year_start=1950, year_end=2021, source_dir='cmip6')
    era5_index = NAM(era5, annual_cycle_fft=2, running_mean=0)
    print(era5_index.index.shape)
